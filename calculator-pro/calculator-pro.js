...
function getNumbers() {
    ...
    if (isNaN(numbersToCalc)) {
        ...
        if (isNaN(getUserValues)) {
            ...
            finalResult.push(`La raiz cuadrada de ${getUserValues} es: ${userValues}`);
            return showResult();
        }
    } else {
        let counter = 0
        while (counter < numbersToCalc) {
            const getUserValues = prompt(`Introduce el ${counter + 1}º Numero \nVolver[x]`);
...
function doCalc() {
    ...
    for (const num of arguments) {
        ...
        for (let y = 0; y < calcUserResult.length; y++) {
            calcUserResult[y] - Math.floor(calcUserResult[y]) == 0 ? userCalcLastCheck.push(calcUserResult[y]) : userCalcLastCheck.push(Number(calcUserResult[y].toFixed(2)));
        }
    }
    //
    let z = 0;
