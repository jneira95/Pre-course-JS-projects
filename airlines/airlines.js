...
function wellcome() {
    ...
    for (let i = 0; i < flights.length; i++) {
        ...
        if (arrFly.scale === false) {
            console.log("\t- El vuelo con origen " + `%c${arrFly.from}` + " %cy destino " + `%c${arrFly.to}` + " %ctiene un coste de " + `%c${arrFly.cost + "€"}` + " %cy no realiza ninguna escala.", "color: orange", "", "color: orange", "", "color: orange", "")
        } else {
            flyWithScale += 1;
            console.log("\t- El vuelo con origen " + `%c${arrFly.from}` + " %cy destino " + `%c${arrFly.to}` + " %ctiene un coste de " + `%c${arrFly.cost + "€"}` + " %cy realiza escala.", "color: orange", "", "color: orange", "", "color: orange", "")
        }
        averageCost += arrFly.cost
    }
    const lastFly = flights.slice(flights.length - 5)
    console.log(`\n${userNameCap} este es el precio medio de los vuelos es %c${(averageCost / flights.length).toFixed(2) + "€"}. \n%cUn total de %c${flyWithScale} %cvuelos realizan escala.`, "color: orange", "", "color: orange", "");
